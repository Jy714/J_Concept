...
const Add = ({token}) => {
  ...
  const onSubmitHandler = async (e) => { 
    ...
    try {
      ...
      // check if we need to upload the image (if we haven't upload then no need to submit the null one)
    ...
    <form onSubmit={onSubmitHandler} className='flex flex-col w-full items-start gap-3'>
      ...
      <div className='flex flex-col sm:flex-row gap-2 w-full sm:gap-8'>
        ...
        <div>
          ...
          <input onChange={(e)=> setPrice(e.target.value)} value={price} className='w-full px-3 py-2 sm:w-[120px]' type="number" required min={1} placeholder='25' />
        </div>

      </div>

      <div>
        <p className='mb-2'>Product Sizes (US)</p>
        <div className='flex gap-3'>
          ...
          <div onClick={() => setSizes(prev => prev.includes("11") ? prev.filter(item => item !== "11") : [...prev,"11"])}>
            <p className={`${sizes.includes("11") ? "bg-pink-100" : "bg-slate-200"} px-3 py-1 cursor-pointer`}>11</p>
          </div>
        </div>
      </div>
